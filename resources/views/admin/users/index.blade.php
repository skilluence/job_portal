...
@section('module-description', 'Manage admin, recruiter, and team manager accounts.')
@section('content')
        ...
        <div class="d-flex gap-8" style="flex-wrap:wrap;align-items:center;">
            <form method="GET" action="{{ route('admin.users') }}" class="d-flex gap-8" style="flex-wrap:wrap;">
                ...
                <select name="role" class="form-control" style="width:160px;" onchange="this.form.submit()">
                    ...
                    @foreach (['admin' => 'Admin', 'recruiter' => 'Recruiter', 'manager' => 'Team Manager'] as $key => $label)
                        <option value="{{ $key }}" @selected($role === $key)>{{ $label }}</option>
                    @endforeach
                </select>
                <button type="submit" class="btn btn-outline"><i class="bi bi-search"></i></button>
                @if ($search || $role)
                    <a href="{{ route('admin.users') }}" class="btn btn-outline" title="Clear filters">
        ...
        <form method="POST" action="{{ route('admin.users.store') }}">
            ...
            <div class="modal-body">
                ...
                <div class="form-group mb-0">
                    <label class="form-label">Team Manager (Admin default for Recruiters)</label>
                    <select name="team_manager_id" class="form-control">
                        <option value="">Auto assign default</option>
                        @foreach ($managers as $manager)
                            <option value="{{ $manager->id }}" @selected(old('team_manager_id', $defaultAdmin?->id) == $manager->id)>
                                {{ $manager->name }} ({{ $manager->role_label ?? ucfirst($manager->role) }})
                            </option>
                        @endforeach
                    </select>
                </div>
        ...
        <form method="POST" id="editUserForm" action="" data-base="{{ url('admin/users') }}">
            ...
            <div class="modal-body">
                ...
                <div class="form-group mb-0">
                    ...
                    <select name="team_manager_id" id="edit_user_team_manager_id" class="form-control">
                        <option value="">Auto assign default</option>
                        @foreach ($managers as $manager)
                            <option value="{{ $manager->id }}">{{ $manager->name }} ({{ $manager->role_label ?? ucfirst($manager->role) }})</option>
                        @endforeach
                    </select>
                </div>
            </div>
